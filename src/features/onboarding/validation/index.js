...
export const PAN_RULES = [
  ...
  {
    label: "5 letters + 4 numbers + 1 letter",
    test: (v) => /^[A-Z]{5}[0-9]{4}[A-Z]$/.test(v.toUpperCase()),
  },
  { label: "No special characters", test: (v) => /^[A-Z0-9]+$/i.test(v) },
  {
    label: "Must be a Business PAN",
    test: (v) => v.length >= 4 && v[3] !== "P",
  },
];
...
export const GST_RULES = [
  ...
  {
    label: "Valid GST format",
    test: (v) => /^\d{2}[A-Z]{5}\d{4}[A-Z][1-9A-Z]Z[0-9A-Z]$/i.test(v),
  },
  {
    label: "State code valid",
    test: (v) => parseInt(v.slice(0, 2)) >= 1 && parseInt(v.slice(0, 2)) <= 37,
  },
  {
    label: "PAN embedded matches",
    test: (v, pan) =>
      !!pan &&
      pan.length === 10 &&
      v.slice(2, 12).toUpperCase() === pan.toUpperCase(),
  },
];
...
export const BANK_RULES = [
  {
    label: "Valid account number",
    test: (v) => /^\d{9,18}$/.test(v.accountNumber),
  },
  {
