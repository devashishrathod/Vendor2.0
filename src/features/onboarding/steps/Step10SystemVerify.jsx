...
function ScoreRing({ score }) {
  ...
  return (
    <div className="flex flex-col items-center">
      ...
      <span className="text-[10px] text-gray-400 mt-1">Verification Score</span>
    </div>
  );
}
...
function GearIcon() {
  return (
    <svg className="w-10 h-10 text-emerald-500 animate-spin" style={{ animationDuration: '2s' }}
...
function buildChecks(flags = {}, remarks = []) {
  ...
  return [
    ...
    },
    {
      ...
      detail: !flags.gstVerified ? 'GST could not be verified' : !flags.gstActive ? 'GST is inactive' : null,
    },
    {
      key: 'panGst',
      label: 'PAN ↔ GST Match',
    ...
    },
    {
      ...
      detail: !flags.bankVerified ? 'Bank could not be verified'
        : !flags.bankMatched ? remark('bank') || 'Bank holder name mismatch' : null,
    },
    {
      key: 'entity',
    ...
    },
    {
      ...
      status: (flags.duplicatePAN || flags.duplicateGST || flags.duplicateBank || flags.duplicateWhatsapp)
        ? 'warn' : 'done',
      detail: flags.duplicateWhatsapp ? remark('duplicate') || 'Duplicate merchant details detected' : null,
    },
  ];
}
...
export default function Step10SystemVerify({ onSuccess }) {
  ...
  const [phase,       setPhase]       = useState('loading'); // loading | success | review | failed | error
  ...
  // Animate checks one-by-one from API result
  ...
  const runVerification = async () => {
    ...
    try {
      ...
      const s = data.status;
      if (s === 'APPROVED')  setPhase('success');
      else if (s === 'REVIEW') setPhase('review');
  ...
  // Derived colors per phase
  const phaseConfig = {
    success: {
      iconBg: 'bg-emerald-100', iconColor: 'text-emerald-500',
      cardBg: 'bg-emerald-50', cardBorder: 'border-emerald-100',
      title: 'Verification Passed', subtitle: 'All checks completed successfully.',
    },
    review: {
      iconBg: 'bg-amber-100', iconColor: 'text-amber-500',
      cardBg: 'bg-amber-50', cardBorder: 'border-amber-100',
      title: 'Under Review', subtitle: 'Some items need manual review. You can continue.',
    },
    failed: {
      iconBg: 'bg-red-100', iconColor: 'text-red-500',
      cardBg: 'bg-red-50', cardBorder: 'border-red-100',
      title: 'Verification Failed', subtitle: 'Some details could not be verified. Please review and retry.',
    },
    error: {
      iconBg: 'bg-red-100', iconColor: 'text-red-500',
      cardBg: 'bg-red-50', cardBorder: 'border-red-100',
      title: 'Connection Error', subtitle: 'Failed to reach the verification server.',
    },
  };
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-white via-emerald-50/30 to-white">
      <div className="w-full max-w-sm mx-4"
          ...
          <>
            ...
            <div className="text-center mb-6">
              ...
              <p className="text-xs text-gray-400 leading-relaxed">
                Cross-checking your PAN, GST, and Bank details…
              </p>
            </div>
            <div className="bg-gray-50 border border-gray-100 rounded-2xl px-5 py-2 mb-6">
              {checks.length === 0
                ...
                : checks.map(c => <CheckRow key={c.key} label={c.label} status={c.status} detail={c.detail} />)
              }
            </div>
          </>
        )}

        {/* ── RESULT (success / review / failed / error) ── */}
        {phase !== 'loading' && cfg && (
          <>
            {/* Icon + score row */}
            <div className="flex items-center justify-between mb-5 px-2">
              <div className={`w-16 h-16 rounded-full ${cfg.iconBg} flex items-center justify-center`}
                ...
                {phase === 'success' && (
                  <svg className={`w-8 h-8 ${cfg.iconColor}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div className="mb-5">
              <h2 className="text-xl font-bold text-gray-900 mb-1">{cfg.title}</h2>
              <p className="text-xs text-gray-400 leading-relaxed">{cfg.subtitle}</p>
            </div>

            {/* Checks card */}
            {checks.length > 0 && (
              <div className={`${cfg.cardBg} border ${cfg.cardBorder} rounded-2xl px-5 py-2 mb-5`}>
                {checks.map(c => <CheckRow key={c.key} label={c.label} status={c.status} detail={c.detail} />)}
              </div>
            )}
              ...
              <div className="bg-amber-50 border border-amber-100 rounded-xl px-4 py-3 mb-5">
                ...
                {sysData.remarks.map((r, i) => (
                  <div key={i} className="flex items-start gap-1.5 mb-1 last:mb-0">
                    ...
                    <span className="text-[11px] text-amber-700 leading-snug">{r}</span>
                  </div>
                ))}
              </div>
            )}
            ...
            {(phase === 'failed' || phase === 'error') && (
              <div className="flex gap-3">
                <button
                  onClick={() => goToStep(STEPS.BUSINESS_VERIFICATION)}
                  className="flex-1 py-3 rounded-xl border-2 border-gray-200 bg-white hover:bg-gray-50
                    text-gray-600 text-sm font-bold transition-all duration-200 active:scale-[0.98]
        ...
        <p className="text-center text-xs text-gray-300 mt-5">Step 10 of 13</p>
      </div>

      {/* ── Error Modal (network / server errors) ── */}
      <ErrorModal
