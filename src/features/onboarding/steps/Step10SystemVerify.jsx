...
function buildChecks(flags = {}, remarks = []) {
  const remark = (key) =>
    remarks.find((r) => r.toLowerCase().includes(key)) || null;
  ...
  return [
    ...
    },
    {
      ...
      detail: !flags.panMatchedWithGST
        ? remark("pan") || "PAN and GST name mismatch"
        : null,
    },
    {
      key: "bank",
      label: "Bank Account Verified",
      status: !flags.bankVerified
        ? "failed"
        : !flags.bankMatched
          ? "warn"
          : "done",
    ...
    },
    {
      ...
      detail: !flags.businessEntityMatched
        ? "Entity type mismatch between PAN and GST"
        : null,
    },
    {
      key: "duplicate",
...
export default function Step10SystemVerify({ onSuccess }) {
  ...
  const phaseConfig = {
    ...
    failed: {
      ...
      subtitle: "Some details could not be verified. Please review and retry.",
    },
    error: {
      iconBg: "bg-red-100",
      iconColor: "text-red-500",
          ...
          <>
            ...
            <div className="flex items-center justify-between mb-5 px-2">
              ...
              >
                {phase === "success" && (
                  <svg
                    className={`w-8 h-8 ${cfg.iconColor}`}
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                  >
            ...
            {checks.length > 0 && (
              <div
                className={`${cfg.cardBg} border ${cfg.cardBorder} rounded-2xl px-5 py-2 mb-5`}
              >
                {checks.map((c) => (
                  <CheckRow
                    key={c.key}
