import { useOnboardingStore, BIZ_SUB, BANK_SUB } from "@/features/onboarding/store/onboardingStore";
...
function VerificationToast({ registrationStatus, taxpayerType, message, isActive }) {
...
export default function Step9GSTReadOnly() {
  ...
  const d = {
    ...
    natureOfBusiness: Array.isArray(raw.natureOfBusiness) && raw.natureOfBusiness.length > 0
    ...
    address: (() => {
      ...
      if (!a) return null;
      const parts = [
        a.building_number, a.building_name, a.location,
        a.city, a.district, a.state, a.pin,
      ].filter(v => v && v.trim() !== '' && v !== 'NA');
      return a.location && a.location.trim() !== '' ? a.location : parts.join(', ') || null;
    })(),
  };

  console.log('Step9 — final d:', d);

  const isActive = ['Active', 'SUCCESS'].includes(d.registrationStatus) ||
  ...
  const normalizeTaxpayerType = (val) => {
    ...
    const map = {
      'REGULAR': 'REGULAR', 'COMPOSITION': 'COMPOSITION', 'SEZ_UNIT': 'SEZ_UNIT',
      'SEZ_DEVELOPER': 'SEZ_DEVELOPER', 'INPUT_SERVICE_DISTRIBUTOR': 'INPUT_SERVICE_DISTRIBUTOR',
      'ISD': 'INPUT_SERVICE_DISTRIBUTOR', 'TAX_DEDUCTOR': 'TAX_DEDUCTOR',
      'TAX_COLLECTOR': 'TAX_COLLECTOR', 'CASUAL_TAXABLE_PERSON': 'CASUAL_TAXABLE_PERSON',
      'NON_RESIDENT_TAXABLE_PERSON': 'NON_RESIDENT_TAXABLE_PERSON',
      'GOVERNMENT_DEPARTMENT': 'GOVERNMENT_DEPARTMENT', 'UN_BODY': 'UN_BODY',
      'EMBASSY': 'EMBASSY', 'OIDAR': 'OIDAR', 'REGISTERED_PERSON': 'REGISTERED_PERSON',
    };
  ...
  const handleContinue = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        natureOfBusiness: Array.isArray(verifyData.natureOfBusiness) && verifyData.natureOfBusiness.length > 0
          ? verifyData.natureOfBusiness : undefined,
        ...
        address: verifyData.address &&
          Object.values(verifyData.address).some(v => v && String(v).trim() !== '')
          ? {
      ...
      const res = await fetch(`${baseUrl}brands/onboarding/add-gst-details`, {
        method: 'POST',
        headers: {
      ...
      setPostError(err.message || 'Failed to save GST details. Please try again.');
    } finally {
      setPosting(false);
    }
  };
  ...
  return (
    <>
      ...
      <div className="w-full" style={{ animation: 'stepIn 0.35s cubic-bezier(0.34,1.4,0.64,1) both' }}>
        ...
        {/* GSTIN Card */}
        <div className="bg-gradient-to-r from-blue-500 to-blue-600 rounded-2xl px-5 py-4 mb-4
          flex items-center justify-between shadow-md shadow-blue-100">
          <div>
            <p className="text-blue-100 text-[10px] font-semibold uppercase tracking-widest mb-1">GSTIN</p>
            <p className="text-white text-lg font-mono font-bold tracking-[0.15em]">{d.gstNumber}</p>
          </div>
        ...
        <div className="bg-white border border-gray-100 rounded-2xl px-4 py-1 shadow-sm mb-4">
          ...
          <DetailRow label="Constitution of Business" value={d.constitutionOfBusiness} />
          <DetailRow label="Taxpayer Type" value={d.taxpayerType} />
          <DetailRow label="Registration Date" value={d.registrationDate} />
          {d.cancellationDate && <DetailRow label="Cancellation Date" value={d.cancellationDate} />}
          {d.natureOfBusiness && <DetailRow label="Nature of Business" value={d.natureOfBusiness} />}
        ...
        <div className="bg-amber-50 border border-amber-100 rounded-xl px-4 py-3
          ...
          <p className="text-xs text-amber-700 leading-relaxed">
            Please verify these details carefully. Incorrect GST details may cause issues during onboarding & tax processing.
          </p>
        </div>
