...
export default function OnboardingPage() {
  ...
  function resolveComponent(step, subStep) {
    if (step === STEPS.BASIC_DETAILS) {
      ...
      if (subStep === BASIC_SUB.REGISTRATION_STATUS)      return <Step4IsRegistered />;
      if (subStep === BASIC_SUB.REGISTRATION_ENTITY_TYPE) return <Step5BusinessType />;
    }
    if (step === STEPS.BUSINESS_VERIFICATION) {
      if (subStep === BIZ_SUB.PAN_VERIFICATION) return <Step6PANEnter />;
      if (subStep === BIZ_SUB.PAN_READONLY)     return <Step7PANReadOnly panData={panDetails} />;
    ...
    if (step === STEPS.BANK_VERIFICATION) {
      if (subStep === BANK_SUB.BANK_VERIFICATION) return (
        <Step11BankEnter
          onFetchSuccess={(data) => {
      ...
      if (subStep === BANK_SUB.BANK_READONLY) return (
