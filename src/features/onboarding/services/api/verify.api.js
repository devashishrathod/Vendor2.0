...
async function backendRequest(path, { method = 'GET', body } = {}) {
  ...
  if (!res.ok) {
    if (Array.isArray(data?.error)) throw new Error(data.error.join(', '));
    const err        = new Error(data?.message || data?.error || `Error ${res.status}: ${res.statusText}`);
...
export async function verifyPAN(pan) {
  if (!pan || typeof pan !== 'string') throw new Error('PAN must be a non-empty string.');
...
export async function verifyGST(gstin) {
  if (!gstin || typeof gstin !== 'string') throw new Error('GSTIN must be a non-empty string.');
