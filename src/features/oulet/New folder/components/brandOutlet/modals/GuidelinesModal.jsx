    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm" onClick={onClose}>
      ...
      >
        ...
        <div className="px-6 py-5 space-y-5">
          {g.sections.map((s, i) => (
            <div key={i}>
              <p className="text-sm font-bold text-gray-800 dark:text-gray-100 mb-1">{s.heading}</p>
              <p className="text-sm text-gray-600 dark:text-gray-300 whitespace-pre-line leading-relaxed">{s.body}</p>
            </div>
          ))}
        </div>
