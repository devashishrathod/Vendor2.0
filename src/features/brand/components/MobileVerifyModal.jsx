...
//   1. Confirm/change the number → sendMobileVerification, which returns a
//      sessionId this modal must hand back on step 2.
//   2. Enter the OTP that was sent → verifyMobileOtp({ mobile, otp, sessionId }).
export default function MobileVerifyModal({ currentMobile, onClose, onVerified }) {
  ...
  const handleVerify = async (e) => {
    ...
    setError("");
    try {
      await verifyMobileOtp({ mobile: mobile.trim(), otp: otp.trim(), sessionId });
      setSuccess(true);
      await onVerified?.();
    } catch (err) {
